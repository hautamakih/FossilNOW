...
def create_map_figure(gdff, genera):
    if len(gdff[genera].unique()) <= 8:
        ...
        fig = px.scatter_mapbox(gdff, lat=gdff.geometry.y, lon=gdff.geometry.x, mapbox_style="open-street-map", zoom=3, color=genera, hover_data=["COUNTRY", "NAME", "MIN_AGE", "MAX_AGE"])
    else:
        fig = px.scatter_mapbox(gdff, lat=gdff.geometry.y, lon=gdff.geometry.x, mapbox_style="open-street-map", zoom=3, color=genera, hover_data=["COUNTRY", "NAME", "MIN_AGE", "MAX_AGE"])
